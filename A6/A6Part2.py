...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    stdDev = np.zeros(len(f0cents))
    for f in range(winStable-1, f0cents.size):
        stdDev[f] = np.std(f0cents[f-(winStable-1): f+1])


    #3. apply threshold on standard deviation values to find indexes of the stable points in melody
    stablePoints = np.where(stdDev < stdThsld)[0]
    stablePoints = stablePoints[winStable:]
    ...
    startIdx = stablePoints[0]
    endIdx = stablePoints[0]

    for i in range(1, stablePoints.size):
        if stablePoints[i] == stablePoints[i-1]+1:
            endIdx = stablePoints[i]
        else:
            segments = np.append(segments, [[startIdx, endIdx]], axis=0)

            #print([startIdx,endIdx])
            startIdx = stablePoints[i]
            endIdx = startIdx

    #5. apply segment filtering, i.e. remove segments with are < minNoteDur in length
    minNoteDurSamples = int(minNoteDur * fs / H)

    segToKeep = np.where((segments[:,1]-segments[:,0])>=minNoteDurSamples)[0]
    segments = segments[segToKeep,:]
    # plotSpectogramF0Segments(x, fs, w, N, H, f0, segments)  # Plot spectrogram and F0 if needed
    # return segments
    return segments

def deleteArray(arrtoDel, arr):
    index = 0
    size = len(arr)
    while index < size:
        if np.array_equal(arr[index], arrtoDel):
            arr.pop(index)
            break
        else:
            index += 1
