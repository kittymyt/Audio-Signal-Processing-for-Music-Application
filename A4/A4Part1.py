...
def extractMainLobe(window, M):
    ...
    N = M*8
    hN = int(N/2)
    fftbuffer = np.zeros(N)                # zero phase window
    fftbuffer[:hM1] = w[hM2:]
    ...
    # find the minimas
    peak = np.where(abs(mX - max(mX)) < eps)[0][0]   # indice of peak
    minimuses = argrelextrema(mX, np.less)             # the minimus of the whole mX
    index = (np.abs(minimuses[0]-peak)).argmin()   # nth of element(index number) in minimuses
    
    #find the left and right minimas index
    if minimuses[0][index] < peak:
        leftMinimasIndex = minimuses[0][index]
        rightMinimasIndex = minimuses[0][index+1]
    else:
        leftMinimasIndex = minimuses[0][index-1]
        rightMinimasIndex = minimuses[0][index]

    #return mX,peak,leftMinimasIndex,rightMinimasIndex
    return mX[leftMinimasIndex:rightMinimasIndex+1]